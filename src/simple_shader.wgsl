...
// Вершинные атрибуты для простой геометрии
struct VertexInput {
...
fn vs_main(vertex: VertexInput) -> VertexOutput {
    ...
    // Простое преобразование координат - геометрия уже в NDC
    ...
    if (vertex.element_type < 0.5) {
        ...
        if (vertex.color_type > 0.5) {
            out.color = uniforms.bullish_color; // Зеленый для бычьих
        } else {
            out.color = uniforms.bearish_color;  // Красный для медвежьих
        }
    } else if (vertex.element_type < 1.5) {
        // Фитили свечей
        out.color = uniforms.wick_color; // Серый
        ...
        // Сетка графика
        out.color = vec4<f32>(0.3, 0.3, 0.3, 0.3); // Полупрозрачный серый
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    // Простой fragment shader - возвращаем цвет от vertex shader
