...
fn volume_candle_position_sync() {
    ...
    assert_eq!(last_x, 1.0, 
        "Последний элемент должен быть точно в x=1.0, получено x={:.10}", last_x);
}
...
fn volume_width_sync() {
    ...
    for i in 0..visible_len {
        ...
        assert!(x - half_width >= -1.0, 
            "Левая граница элемента {} выходит за границы: {:.6}", i, x - half_width);
        assert!(x + half_width <= 1.0, 
            "Правая граница элемента {} выходит за границы: {:.6}", i, x + half_width);
    }
}
...
fn debug_positioning_logic() {
    ...
    for (i, (candle_x, volume_x)) in candle_positions.iter().zip(volume_positions.iter()).enumerate() {
        assert!((candle_x - volume_x).abs() < f32::EPSILON,
    ...
    if !candle_positions.is_empty() {
        ...
        assert_eq!(*last_candle, 1.0, 
            "Последняя свеча должна быть в x=1.0, получено {:.10}", last_candle);
        assert_eq!(*last_volume, 1.0,
            "Последний volume bar должен быть в x=1.0, получено {:.10}", last_volume);
    }
    
    // Дополнительная проверка: все позиции должны быть монотонно возрастающими
    for i in 1..candle_positions.len() {
        assert!(candle_positions[i] > candle_positions[i-1],
            "Позиции свечей должны возрастать: pos[{}]={:.6} should be > pos[{}]={:.6}",
            i, candle_positions[i], i-1, candle_positions[i-1]);
    }
} 