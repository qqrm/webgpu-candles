...
fn positioning_regression_basic() {
    ...
    for i in 1..visible {
        assert!(first < candle_x_position(i, visible),
            "First position {:.6} should be less than position {} ({:.6})",
            first, i, candle_x_position(i, visible));
    }
}
...
fn viewport_bounds_regression() {
    ...
    for &size in &test_sizes {
        ...
        assert!(first >= -1.0, 
            "First position {:.6} should be >= -1.0 for size {}", first, size);
        
        // Последняя позиция должна быть точно 1.0
        let last = candle_x_position(size - 1, size);
        assert_eq!(last, 1.0, 
            "Last position should be exactly 1.0 for size {}, got {:.10}", size, last);
        ...
        if size > 1 {
            ...
            assert!((total_span - expected_span).abs() < 1e-6,
                "Total span should be {:.6}, got {:.6} for size {}",
                expected_span, total_span, size);
        }
    }
}
...
fn spacing_uniformity_regression() {
    ...
    for &size in &sizes {
        ...
        for i in 1..size {
            ...
            assert!((actual_step - expected_step).abs() < f32::EPSILON,
                "Step size mismatch for size {} between positions {} and {}: expected {:.6}, got {:.6}",
                size, i-1, i, expected_step, actual_step);
        }
    }
} 